...
export class CurvesModule extends LitModule {
    ...
    for (const modelName of this.appState.currentModels) {
      ...
      if (this.predKey in modelSpec.output) {
        if (modelSpec.output[this.predKey].vocab != null) {
          ...
          if (modelSpec.output[this.predKey].null_idx != null) {
            for (let i = 0; i < modelSpec.output[this.predKey].vocab!.length;
                 i++) {
              if (i !== modelSpec.output[this.predKey].null_idx) {
                this.selectedPositiveLabelIndex = i;
              }
            }
          }
  ...
  renderPositiveLabelSelect() {
    ...
    const renderOption = (label: string, selected: boolean) =>
      selected ? html`<option value=${label} selected>${label}</option>` :
